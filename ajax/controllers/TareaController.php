...
class TareaController {
    ...
    public function cambiarEstadoAjax(): void {
        ...
        $id     = $_POST['tarea_id'] ?? null;
        $estado = $_POST['estado']   ?? null;
        $user   = Auth::getUser();

        if (!$id || !in_array($estado, ['pendiente', 'completada'])) {
            echo json_encode(['success' => false, 'message' => 'Datos inválidos']);
            exit;
        }

        $tareaModel = new Tarea();
        $tarea      = $tareaModel->obtenerPorId((int) $id, $user['id']);
        ...
        $tareaModel->cambiarEstado((int) $id, $user['id'], $estado);
